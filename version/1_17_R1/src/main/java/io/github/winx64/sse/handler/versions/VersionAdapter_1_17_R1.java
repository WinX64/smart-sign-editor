...
public final class VersionAdapter_1_17_R1 implements VersionAdapter {

    @Override
    public void openSignEditor(Player player, Location location) {
        BlockPosition pos = new BlockPosition(location.getX(), location.getY(), location.getZ());
        EntityPlayer nmsPlayer = ((CraftPlayer) player).getHandle();
        TileEntitySign tileEntitySign = (TileEntitySign) Objects.requireNonNull(nmsPlayer.getWorld().getTileEntity(pos));
        PlayerConnection conn = nmsPlayer.b;

        tileEntitySign.f = true;
        tileEntitySign.a(nmsPlayer.getUniqueID());
